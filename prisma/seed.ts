  ...
  for (const clientSeed of seedClients) {
    const record = await prisma.client.upsert({
      ...
      update: {
        ...
        acn: clientSeed.acn,
        tfn: clientSeed.tfn,
        incorporationType: IncorporationType[clientSeed.incorporationType as keyof typeof IncorporationType],
  ...
  for (const applicationSeed of seedApplications) {
    ...
    const application = await prisma.incomeYearApplication.upsert({
      ...
      update: {
        ...
        registrationStatus: RegistrationStatus[applicationSeed.registrationStatus as keyof typeof RegistrationStatus],
  ...
  for (const projectSeed of seedProjects) {
    const project = await prisma.rDProject.upsert({
      ...
      update: {
        ...
        status: ProjectStatus[projectSeed.status as keyof typeof ProjectStatus],
